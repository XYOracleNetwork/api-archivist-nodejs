...
const validateAfterSanitization = (actual: PrepareBoundWitnessesResult) => {
  actual.sanitized.map((bw) => {
    expect(bw._archive).toBeTruthy()
    expect(bw._client).toBeTruthy()
    expect(bw._hash).toBeTruthy()
    expect(bw._observeDuration).toBeTruthy()
    expect(bw._payloads).toBeUndefined()
    expect(bw._timestamp).toBeTruthy()
    expect(bw._user_agent).toBeTruthy()
    expect(bw.addresses).toBeTruthy()
    expect(bw.payload_hashes).toBeTruthy()
    expect(bw.payload_schemas).toBeTruthy()
    expect(bw.schema).toBeTruthy()
  })
  actual.payloads.map((p) => {
    expect(p._archive).toBeTruthy()
    expect(p._client).toBeTruthy()
    expect(p._hash).toBeTruthy()
    expect(p._observeDuration).toBeTruthy()
